...
class AlexNet(object):
    ...
    def weights_l2(self):
        ...
        for skip_layer in self.SKIP_LAYER:
            with tf.variable_scope(skip_layer, reuse=True):
                vars.append(tf.get_variable('weights'))
        weights_l2 = tf.add_n([tf.nn.l2_loss(v) for v in vars]) * lambda_ / 2
        return weights_l2
